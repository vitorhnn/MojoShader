...
/* MojoShader change! #include "../SDL_internal.h" */
...
int
MOJOSHADER_internal_snprintf(SDL_OUT_Z_CAP(maxlen) char *text, size_t maxlen, SDL_PRINTF_FORMAT_STRING const char *fmt, ...)
{
    va_list ap;
    int retval;

    va_start(ap, fmt);
    retval = MOJOSHADER_internal_vsnprintf(text, maxlen, fmt, ap);
    va_end(ap);

    return retval;
}

#if 0 /* MojoShader change! #ifdef HAVE_VSNPRINTF */
...
int
MOJOSHADER_internal_vsnprintf(SDL_OUT_Z_CAP(maxlen) char *text, size_t maxlen, const char *fmt, va_list ap)
{
    size_t left = maxlen;
    char *textstart = text;
