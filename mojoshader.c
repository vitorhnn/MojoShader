...
MAKE_SPIRV_EMITTER_DSS(SLT, {
    ...
    output_id(ctx, result);
    output_id(ctx, bool_result);
    output_id(ctx, spv_vectorbroadcast(ctx, rtid, spv_getscalarf(ctx, 1.0f)));
    output_id(ctx, spv_vectorbroadcast(ctx, rtid, spv_getscalarf(ctx, 0.0f)));
});
